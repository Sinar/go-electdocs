...
// 12 - Jenis Undi
// 13 - RAW DM - can be ignored .. orginal data from PDF
...
func LoadPARSaluran(par string) []saluran {
	...
	for _, cols := range records {
		...
		if cols[0] == "Bil" {
			...
			if cols[0] == "" {
				fmt.Println("SKIP ROW since row is empty")
				continue
			} else if cols[0] == "JUMLAH" {
				fmt.Println("SKIP ROW since is metadata. CHECK:", cols[3])
				continue
			}
			// Verify
			if len(cols) != numCols {
